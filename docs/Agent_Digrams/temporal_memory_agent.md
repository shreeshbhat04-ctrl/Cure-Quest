...
> **Document**: `Cure-Quest/docs/temporal_memory_agent.md`
...
The **Temporal Memory Agent** serves as the "short-term clinical memory" of the Cure-Quest platform. Its primary goal is to retrieve and serve the most relevant clinical data (active conditions, recent vitals, current prescriptions) for the current patient. This ensures that every specialist agent (Recipe, Map, Vision) is "grounded" in the patient's specific health context, preventing generic or unsafe advice.
